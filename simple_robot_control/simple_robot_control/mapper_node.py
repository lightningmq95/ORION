...
#         self.L_OCC = 0.85            # Score added when a lidar ray hits an obstacle
#         self.L_FREE = -0.4           # Score subtracted when a ray passes through free space
#         self.MAX_LOG_ODDS = 3.5      # Max confidence barrier (caps memory of old obstacles, sharper erasing of ghosts)
#         self.MIN_LOG_ODDS = -3.5     # Max confidence for free space (allows quick updates if an obstacle appears)
#         self.OCC_THRESHOLD = 0.8     # Score required to draw the cell as a solid wall (100)
#         self.FREE_THRESHOLD = -0.3   # Score required to draw the cell as guaranteed free space (0)

#         self.MAX_TRUSTED_RANGE = 7.0 # Max distance (meters) to trust lidar hits (truncates long-distance blur)
